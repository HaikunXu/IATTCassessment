...
cpue_compare <- function(Path, Legend, Region, Save_Path, rescale = TRUE, ylabel = "CPUE", xlabel = "", ylim, xlim, CV = TRUE, w = 15, h = 6, smooth = TRUE) {
    ...
    if(rescale==TRUE) {
        if(CV==TRUE) {
          ...
          if(smooth == TRUE) {
            f <- ggplot(data = Index) +
              geom_smooth(aes(x = Year, y = Index, color = Legend), se = FALSE, span = 0.1) +
              theme_bw(15) +
