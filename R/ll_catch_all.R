...
#' 
#' \code{ll_catch_all} This function processes the raw LL catch data into the format for Stock Assessment - including catch before 1975
#' 
#' @export

ll_catch_all = function(Grid_Catch, FSR_Catch, Species, last_year, dir) {
    ...
    LL_Catch_SS <- data.frame(LL_Catch) %>%
        ...
        mutate(Year=(YQ-1954)*4+1,seas=1,Catch_CE=0.01) %>% select(Year,seas,Fishery,Catch,Catch_CE)
