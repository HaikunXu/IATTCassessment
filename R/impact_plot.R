...
impact_plot = function(Dir, n_year, BaseName) {
    ...
    for (step in 1:n_step) {
        ...
        files = c(paste0(Dir, "BET_Base_SAC9/ss3.par"), paste0(Dir, "BET_Base_SAC9/go_nohess.bat"), paste0(Dir, "BET_Base_SAC9/starter.ss"), 
            paste0(Dir, "BET_Base_SAC9/forecast.ss"), paste0(Dir, "BET_Base_SAC9/BET-EPO.ctl"), paste0(Dir, "BET_Base_SAC9/BET-EPO.dat"), 
            paste0(Dir, "BET_Base_SAC9/ss3.exe"))
    ...
    myreplist1 = r4ss::SS_output(dir = Dir1, ncols = 400, covar = F)
    
    Dir2 <- paste0(Dir, step_name[1])
    myreplist2 = r4ss::SS_output(dir = Dir2, ncols = 400, covar = F)
    
    Dir3 <- paste0(Dir, step_name[2])
    myreplist3 = r4ss::SS_output(dir = Dir3, ncols = 400, covar = F)
    
    Dir4 <- paste0(Dir, step_name[3])
    myreplist4 = r4ss::SS_output(dir = Dir4, ncols = 400, covar = F)
    
    Dir5 <- paste0(Dir, step_name[4])
    myreplist5 = r4ss::SS_output(dir = Dir5, ncols = 400, covar = F)
    ...
    ggplot(data = SB_dif) + geom_ribbon(aes(x = Year, ymin = 0, ymax = SB), fill = "red") + geom_ribbon(aes(x = Year, ymin = SB, 
        ymax = SB + noDisc_dif), fill = "green") + geom_ribbon(aes(x = Year, ymin = SB + noDisc_dif, ymax = SB + noDisc_dif + 
        noPS_dif), fill = "purple") + geom_ribbon(aes(x = Year, ymin = SB + noDisc_dif + noPS_dif, ymax = SB + noDisc_dif + 
        noPS_dif + noDLL_dif), fill = "blue") + theme_bw() + ylab("Spawning biomass (mt)")
