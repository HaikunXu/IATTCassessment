...
make_kobeplot <- function(Kobe.Out, Slim = c(0, 6), Flim = c(0, 1.6), sd_f, sd_b) {
    ...
    plot(0, 0, xlim = xlim, ylim = ylim, axes = T, xaxs = "i", yaxs = "i", type = "o", col = "black", ylab = "", xlab = "", lwd = 1.25, pch = 19, 
        cex = 0.75, las = 1)
    ...
    arrows(x[length(x)] - 2 * sd_b * x[length(x)], y[length(x)], x[length(x)] + 2 * sd_b * x[length(x)], y[length(x)], angle = 90, code = 3, length = 0.05, 
        lwd = 2)
    arrows(x[length(x)], y[length(x)] - 2 * sd_f * y[length(x)], x[length(x)], y[length(x)] + 2 * sd_f * y[length(x)], angle = 90, code = 3, length = 0.05, 
        lwd = 2)
