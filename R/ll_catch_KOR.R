...
#' 
#' \code{ll_catch_KOR} This function processes the raw LL catch data into the format for Stock Assessment (KOR is considered a seperate fleet)
#' 
#' @export

ll_catch_KOR = function(Grid_Catch, FSR_Catch, Species, last_year, dir) {
  ...
  for (c in 1:length(Countries)) {
    ...
    # new code for ll_catch_KOR
    if(Countries[c] == "KOR") {
      save_KOR <- save
      colnames(save_KOR) <- c("YQ", paste0("N", seq(1, n_areas)), paste0("W", seq(1, n_areas)))
    }
    else save_all <- rbind(save_all, data.matrix(save))
  }
  ...
  write.csv(save_KOR, paste0(dir, "save_KOR.csv"), row.names = FALSE)
  ...
  LL_Catch_KOR <- data.frame(save_KOR) %>% gather(c(paste0("N", seq(1, n_areas)), paste0("W", seq(1, n_areas))), key = "term", 
                                                  value = "catch") %>% group_by(YQ, term) %>% summarise(tot_catch = sum(catch, na.rm = T)) %>% spread(key = term, 
                                                                                                                                                      value = tot_catch) %>% filter(YQ >= 1975)
  ####################### Coastal countries
  ...
  write.csv(LL_Catch, paste0(dir, "LL_Catch_noKOR.csv"), row.names = FALSE)
  write.csv(LL_Catch_SS, paste0(dir, "LL_Catch_noKOR_SS.csv"), row.names = FALSE)
  
  LL_Catch_KOR_SS <- data.frame(LL_Catch_KOR) %>%
    gather(c(paste0("N", seq(1, n_areas)), paste0("W", seq(1, n_areas))), key = "Fishery",value = "Catch") %>%
    mutate(Year=(YQ-1975)*4+1,seas=1,Catch_CE=0.01) %>% select(Year,seas,Fishery,Catch,Catch_CE)
  
  write.csv(LL_Catch_KOR, paste0(dir, "LL_Catch_KOR.csv"), row.names = FALSE)
  write.csv(LL_Catch_KOR_SS, paste0(dir, "LL_Catch_KOR_SS.csv"), row.names = FALSE)
  
  # LL_Catch_all <- LL_Catch_KOR + LL_Catch
  # LL_Catch_all_SS <- data.frame(LL_Catch_all) %>%
  #   gather(c(paste0("N", seq(1, n_areas)), paste0("W", seq(1, n_areas))), key = "Fishery",value = "Catch") %>%
  #   mutate(Year=(YQ-1975)*4+1,seas=1,Catch_CE=0.01) %>% select(Year,seas,Fishery,Catch,Catch_CE)
  # 
  # write.csv(LL_Catch_all, paste0(dir, "LL_Catch_all.csv"), row.names = FALSE)
  # write.csv(LL_Catch_all_SS, paste0(dir, "LL_Catch_all_SS.csv"), row.names = FALSE)
  ...
  ggplot(data=LL_Catch_plot) +
    ...
    theme_bw()
  ggsave(filename = paste0(dir,"noKOR_Catches.png"), dpi = 300, width = 8, height = 8)
  
  LL_Catch_KOR_plot <- data.frame(LL_Catch_KOR) %>%
    gather(c(paste0("N", seq(1, n_areas)), paste0("W", seq(1, n_areas))), key = "Fishery",value = "Catch") %>%
    mutate(Unit = ifelse(substr(Fishery,1,1)=="N","Number","Weight"),
           Area = substr(Fishery,2,2))
  
  ggplot(data=LL_Catch_KOR_plot) +
    geom_line(aes(x=YQ,y=Catch,color=Area),alpha=0.25) +
    geom_smooth(aes(x=YQ,y=Catch,color=Area),span=0.25,se=FALSE) +
    facet_wrap(~Unit, scales="free_y", nrow=2) +
    theme_bw()
  ggsave(filename = paste0(dir,"KOR_Catches.png"), dpi = 300, width = 8, height = 8)
