...
plot_R = function(SS_Dir, lyear, Save_Dir) {
    ...
    R <- data.frame(est = R_est, std = R_std, year = rep(1975:lyear, each = 4), yq = seq(1975, 
        lyear + 0.75, 0.25))
    ...
    for (y in 1975:lyear) {
        ...
        var_y <- R$std[((y - 1975) * 4 + 1):((y - 1974) * 4)] %*% t(R$std[((y - 1975) * 4 + 1):((y - 
            1974) * 4)]) * cor_y
        ...
        R_annual$Std[which(R_annual$year == y)] <- sqrt(matrix(c(1, 1, 1, 1), nrow = 1, ncol = 4) %*% 
            var_y %*% matrix(c(1, 1, 1, 1), nrow = 4, ncol = 1))
    }
    
    R <- R %>% mutate(R = est/mean(est), STD = std/est)
    R_annual <- R_annual %>% mutate(R = Est/mean(Est), STD = Std/Est)
    
    f1 <- ggplot(data = R) + geom_ribbon(aes(x = yq, ymin = R * exp(-1.96 * STD), ymax = R * exp(1.96 * 
        STD)), fill = "grey") + geom_line(aes(x = yq, y = R), size = 1) + geom_point(aes(x = yq, 
        y = R), data = R[seq(1, nrow(R), 4), ]) + theme_bw(12) + xlab("") + ylab("") + geom_hline(yintercept = 1, 
        linetype = "dashed")
    
    f2 <- ggplot(data = R_annual) + geom_ribbon(aes(x = year, ymin = R * exp(-1.96 * STD), ymax = R * 
        exp(1.96 * STD)), fill = "grey") + geom_line(aes(x = year, y = R), size = 1) + geom_point(aes(x = year, 
        y = R)) + theme_bw(12) + xlab("") + ylab("") + geom_hline(yintercept = 1, linetype = "dashed")
