...
plot_R = function(SS_Dir, lyear, Save_Dir) {
    ...
    f1 <- ggplot(data = R) + geom_ribbon(aes(x = yq, ymin = R * exp(-1.96 * STD), ymax = R * exp(1.96 * STD)), fill = "grey") + geom_line(aes(x = yq, 
        y = R), size = 1) + geom_point(aes(x = yq, y = R), data = R[seq(1, nrow(R), 4), ]) + theme_bw(12) + xlab("") + ylab("") + geom_hline(yintercept = 1, 
        linetype = "dashed")
    
    f2 <- ggplot(data = R_annual) + geom_ribbon(aes(x = year, ymin = R * exp(-1.96 * STD), ymax = R * exp(1.96 * STD)), fill = "grey") + geom_line(aes(x = year, 
        y = R), size = 1) + geom_point(aes(x = year, y = R)) + theme_bw(12) + xlab("") + ylab("") + geom_hline(yintercept = 1, linetype = "dashed")
