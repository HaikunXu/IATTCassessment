...
plot_retro = function(SS_Dir, lyear, fyear, Save_Dir, title, xlim, ylim) {
    ...
    for (i in 1:length(lyear)) {
        Myreplist = r4ss::SS_output(dir=SS_Dir[i],ncols=400,covar=F,forecast=FALSE,verbose = FALSE,printstats = FALSE)
        if(i==1) {
    ...
    f1 <- ggplot(data=SBR %>% filter(label1==1)) +
        ...
        labs(x = "", y = "", color = "Year") +
        theme_bw(20) + ggtitle(title) + ggeasy::easy_center_title()
    ...
    #     labs(x = "", y = "") +
    #     theme_bw(20) + ggtitle(title) + labs(fill = "Year")
    # 
