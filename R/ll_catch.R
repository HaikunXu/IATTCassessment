...
ll_catch = function(Grid_Catch, FSR_Catch, Species, last_year, dir) {
    ...
    for (c in 1:length(Countries)) {
        ...
        Grid_weight_quarterly <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == 
            Species) %>% mutate(YQ = Yrr + (Quarter - 1)/4) %>% group_by(YQ, NewAreas) %>% summarise(Tot_Weight_Sum = sum(SumOfWeight, 
            na.rm = TRUE)) %>% spread(key = NewAreas, value = Tot_Weight_Sum, fill = 0)
        
        Grid_number_quarterly <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == 
            Species) %>% mutate(YQ = Yrr + (Quarter - 1)/4) %>% group_by(YQ, NewAreas) %>% summarise(Tot_Number_Sum = sum(SumOfNumber, 
        ...
        Grid_number_annual <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% 
            ...
            group_by(Yrr) %>% mutate(EPO = sum(Tot_Number_Sum, na.rm = TRUE)) %>% spread(key = NewAreas, 
            value = Tot_Number_Sum, fill = 0)
        ...
        Grid_weight_annual <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% 
            ...
            group_by(Yrr) %>% mutate(EPO = sum(Tot_Weight_Sum, na.rm = TRUE)) %>% spread(key = NewAreas, 
            value = Tot_Weight_Sum, fill = 0)
        ...
        Grid_prop_quarterly <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% 
            ...
            select(Yrr, Quarter, NewAreas, Prop) %>% spread(key = NewAreas, value = Prop, fill = 0) %>% 
            mutate(YQ = Yrr + (Quarter - 1)/4)
        
        Grid_prop_num_quarterly <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == 
            Species) %>% group_by(Yrr, Quarter, NewAreas) %>% summarise(Tot_Number_Sum = sum(SumOfNumber, 
            na.rm = TRUE)) %>% group_by(Yrr) %>% mutate(Prop = Tot_Number_Sum/sum(Tot_Number_Sum, 
            na.rm = TRUE)) %>% select(Yrr, Quarter, NewAreas, Prop) %>% spread(key = NewAreas, 
            value = Prop, fill = 0) %>% mutate(YQ = Yrr + (Quarter - 1)/4)
        
        # if the data do not include some areas for the entire period, add 0 catch to those areas to
        # make the format of the dataset consistent among countries
        ...
        FSR_annual <- FSR_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% 
            group_by(Year) %>% summarise(mt = sum(mt))
        
        # flag = 0 (have gridded number data); 1 (gridded weight data < FSR, need to do allocation); 2
        # (gridded weight data > FSR, use gridded weight data); 3 (no gridded number or weight data,
        # need to do allowcation);
        ...
        if (Countries[c] %in% Special_Countires) {
            allocation_flag[flag_id == TRUE] <- ifelse(Grid_number_annual$EPO > 0, ifelse(Grid_weight_annual$EPO > 
                0 & Grid_weight_annual$EPO < FSR_annual[which(FSR_annual$Year %in% Grid_weight_annual$Yrr), 
                "mt"], 1, 0), ifelse(Grid_weight_annual$EPO > 0, ifelse(Grid_weight_annual$EPO < 
        ...
        for (i in 1:length(FSR_annual$Year)) {
            ...
            if (allocation_flag[i] == 1) {
                ...
                allocation[which(allocation$Year == year), paste0("W", seq(1, n_areas))] <- FSR * 
                  Grid_prop_quarterly[which(allocation$Year == year), paste0("", seq(1, n_areas))]
            }
                ...
                if (sum(Grid_prop_quarterly[paste0("", seq(1, n_areas))]) > 0) {
                  ...
                  for (year_diff in 1:40) {
                    ...
                    if ((year - year_diff) %in% Grid_number_annual$Yrr) {
                      if (Grid_weight_annual[which(Grid_weight_annual$Yrr == (year - year_diff)), 
                        "EPO"] > 0) {
                  ...
                  prop <- Grid_prop_quarterly[which(Grid_prop_quarterly$Yrr == year_new), c("Quarter", 
                    paste0("", seq(1, n_areas)))]
                  prop_final <- matrix(0, nrow = 4, ncol = n_areas)
                  for (q in 1:4) {
                    # if no catch in some quarter, add 0 for those quarters to make dataset consistent among
                    # quarters
                    if (q %in% prop$Quarter) 
                      prop_final[q, ] <- data.matrix(prop[which(prop$Quarter == q), 2:(n_areas + 
                        1)])
                  }
                  # print(year_new)
                  allocation[which(allocation$Year == year), paste0("W", seq(1, n_areas))] <- FSR * 
                  ...
                  for (year_diff in 1:40) {
                    if ((year - year_diff) %in% Grid_number_annual$Yrr) {
                      if (Grid_number_annual[which(Grid_number_annual$Yrr == (year - year_diff)), 
                        "EPO"] > 0) {
                        year_new <- year - year_diff
                        break
                      }
                  ...
                  prop_num <- Grid_prop_num_quarterly[which(Grid_prop_num_quarterly$Yrr == year_new), 
                    c("Quarter", paste0("", seq(1, n_areas)))]
                  prop_final <- matrix(0, nrow = 4, ncol = n_areas)
                  for (q in 1:4) {
                    if (q %in% prop_num$Quarter) 
                      prop_final[q, ] <- data.matrix(prop_num[which(prop_num$Quarter == q), 2:(n_areas + 
                        1)])
                  }
                  # print(year_new)
                  allocation[which(allocation$Year == year), paste0("W", seq(1, n_areas))] <- FSR * 
        ...
        save <- right_join(Grid_number_quarterly[, c("YQ", paste0("", seq(1, n_areas)))], allocation[, 
            c(1, seq(4, n_areas + 3))])
        ...
        if (sum(!is.na(save[seq(nrow(save) - 3, nrow(save)), 2:(1 + 2 * n_areas)])) == 0) {
            # print(Countries[c])
            save[seq(nrow(save) - 3, nrow(save)), 2:(1 + 2 * n_areas)] <- save[seq(nrow(save) - 
                3 - 4, nrow(save) - 4), 2:(1 + 2 * n_areas)]
        }
        
        # remove those numbers where weight allocation exist (see rule#2 in the word file)
        save[, paste0("", seq(1, n_areas))] <- save[, paste0("", seq(1, n_areas))] * ifelse(is.na(save[, 
            paste0("W", seq(1, n_areas))]), 1, NA)
    ...
    LL_Catch <- data.frame(save_all) %>% gather(c(paste0("N", seq(1, n_areas)), paste0("W", seq(1, 
        n_areas))), key = "term", value = "catch") %>% group_by(YQ, term) %>% summarise(tot_catch = sum(catch, 
        na.rm = T)) %>% spread(key = term, value = tot_catch) %>% filter(YQ >= 1975)
    
    
    ####################### Coastal countries
    Coastal_Countries <- as.character(unique(FSR_Catch$FlagAbv))[!as.character(unique(FSR_Catch$FlagAbv)) %in% 
        Countries]
    ...
    if (sum(Coastal_Catch[(nrow(LL_Catch) - 3):nrow(LL_Catch), 2:(n_areas + 1)]) == 0) {
        Coastal_Catch[(nrow(LL_Catch) - 3):nrow(LL_Catch), 2:(n_areas + 1)] <- Coastal_Catch[(nrow(LL_Catch) - 
            3 - 4):(nrow(LL_Catch) - 4), 2:(n_areas + 1)]
    }
    
    write.csv(Coastal_Catch, paste0(dir, "Coastal_Catch.csv"), row.names = FALSE)
    
    # total catch
    LL_Catch[, paste0("W", seq(1, n_areas))] <- LL_Catch[, paste0("W", seq(1, n_areas))] + Coastal_Catch[, 
        2:(n_areas + 1)]
