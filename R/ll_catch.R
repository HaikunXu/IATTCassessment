...
ll_catch = function(Grid_Catch, FSR_Catch, Species, last_year, dir) {
    ...
    for (c in 1:length(Countries)) {
        ...
        Grid_prop_quarterly <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% group_by(Yrr, 
            ...
            mutate(Prop = Tot_Weight_Sum/sum(Tot_Weight_Sum, na.rm = TRUE)) %>% select(Yrr, Quarter, NewAreas, 
        ...
        Grid_prop_num_quarterly <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% group_by(Yrr, 
            ...
            mutate(Prop = Tot_Number_Sum/sum(Tot_Number_Sum, na.rm = TRUE)) %>% select(Yrr, Quarter, NewAreas, 
            Prop) %>% spread(key = NewAreas, value = Prop, fill = 0) %>% mutate(YQ = Yrr + (Quarter - 1)/4)
        
        # if the data do not include some areas for the entire period, add 0 catch to those areas to make the format
        # of the dataset consistent among countries
    ...
    LL_Catch <- data.frame(save_all) %>% gather(c(paste0("N", seq(1, n_areas)), paste0("W", seq(1, n_areas))), 
        key = "term", value = "catch") %>% group_by(YQ, term) %>% summarise(tot_catch = sum(catch, na.rm = T)) %>% 
        spread(key = term, value = tot_catch) %>% filter(YQ >= 1975)
