...
ll_catch = function(Grid_Catch, FSR_Catch, Species, last_year, dir) {
    ...
    ggplot() + geom_point(aes(x = Lon, y = Lat, color = Areas), data = data, size = 6, shape = 15) + geom_polygon(data = wmap, aes(long, lat, group = group), 
        fill = "black", colour = "white", alpha = 1, lwd = 0.5) + coord_quickmap(ylim = c(-40, 40), xlim = c(-150, -70)) + theme_bw(8)
    ...
    for (c in 1:length(Countries)) {
        ...
        Grid_weight_quarterly <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% mutate(YQ = Yrr + (Quarter - 1)/4) %>% 
            group_by(YQ, NewAreas) %>% summarise(Tot_Weight_Sum = sum(SumOfWeight, na.rm = TRUE)) %>% spread(key = NewAreas, value = Tot_Weight_Sum, 
            fill = 0)
        
        Grid_number_quarterly <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% mutate(YQ = Yrr + (Quarter - 1)/4) %>% 
            group_by(YQ, NewAreas) %>% summarise(Tot_Number_Sum = sum(SumOfNumber, na.rm = TRUE)) %>% spread(key = NewAreas, value = Tot_Number_Sum, 
            fill = 0)
        
        Grid_number_annual <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% group_by(Yrr, NewAreas) %>% summarise(Tot_Number_Sum = sum(SumOfNumber, 
            na.rm = TRUE)) %>% group_by(Yrr) %>% mutate(EPO = sum(Tot_Number_Sum, na.rm = TRUE)) %>% spread(key = NewAreas, value = Tot_Number_Sum, 
            fill = 0)
        
        Grid_weight_annual <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% group_by(Yrr, NewAreas) %>% summarise(Tot_Weight_Sum = sum(SumOfWeight, 
            na.rm = TRUE)) %>% group_by(Yrr) %>% mutate(EPO = sum(Tot_Weight_Sum, na.rm = TRUE)) %>% spread(key = NewAreas, value = Tot_Weight_Sum, 
            fill = 0)
        
        # calculate the prop (in weight and number) by quarter by area
        Grid_prop_quarterly <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% group_by(Yrr, Quarter, NewAreas) %>% summarise(Tot_Weight_Sum = sum(SumOfWeight, 
            na.rm = TRUE)) %>% group_by(Yrr) %>% mutate(Prop = Tot_Weight_Sum/sum(Tot_Weight_Sum, na.rm = TRUE)) %>% select(Yrr, Quarter, NewAreas, 
            Prop) %>% spread(key = NewAreas, value = Prop, fill = 0) %>% mutate(YQ = Yrr + (Quarter - 1)/4)
        
        Grid_prop_num_quarterly <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% group_by(Yrr, Quarter, NewAreas) %>% 
            summarise(Tot_Number_Sum = sum(SumOfNumber, na.rm = TRUE)) %>% group_by(Yrr) %>% mutate(Prop = Tot_Number_Sum/sum(Tot_Number_Sum, na.rm = TRUE)) %>% 
            select(Yrr, Quarter, NewAreas, Prop) %>% spread(key = NewAreas, value = Prop, fill = 0) %>% mutate(YQ = Yrr + (Quarter - 1)/4)
        
        # if the data do not include some areas for the entire period, add 0 catch to those areas to make the format of the dataset consistent among
        # countries
        ...
        FSR_annual <- FSR_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% group_by(Year) %>% summarise(mt = sum(mt))
        
        # flag = 0 (have gridded number data); 1 (gridded weight data < FSR, need to do allocation); 2 (gridded weight data > FSR, use gridded weight
        # data); 3 (no gridded number or weight data, need to do allowcation);
        ...
        if (Countries[c] %in% Special_Countires) {
            allocation_flag[flag_id == TRUE] <- ifelse(Grid_number_annual$EPO > 0, ifelse(Grid_weight_annual$EPO > 0 & Grid_weight_annual$EPO < FSR_annual[which(FSR_annual$Year %in% 
                Grid_weight_annual$Yrr), "mt"], 1, 0), ifelse(Grid_weight_annual$EPO > 0, ifelse(Grid_weight_annual$EPO < FSR_annual[which(FSR_annual$Year %in% 
                Grid_weight_annual$Yrr), "mt"], 1, 2), 3))
        } else {
            allocation_flag[flag_id == TRUE] <- ifelse(Grid_number_annual$EPO > 0, 0, ifelse(Grid_weight_annual$EPO > 0, ifelse(Grid_weight_annual$EPO < 
                FSR_annual[which(FSR_annual$Year %in% Grid_weight_annual$Yrr), "mt"], 1, 2), 3))
        }
        ...
        for (i in 1:length(FSR_annual$Year)) {
            ...
            if (allocation_flag[i] == 1) {
                ...
                allocation[which(allocation$Year == year), paste0("W", seq(1, n_areas))] <- FSR * Grid_prop_quarterly[which(allocation$Year == year), 
    ...
    LL_Catch <- data.frame(save_all) %>% gather(c(paste0("N", seq(1, n_areas)), paste0("W", seq(1, n_areas))), key = "term", value = "catch") %>% 
        group_by(YQ, term) %>% summarise(tot_catch = sum(catch, na.rm = T)) %>% spread(key = term, value = tot_catch) %>% filter(YQ >= 1975)
    ...
    if (Species == "BET") 
        Area_Flag <- c(6, 3, 3, 4, 3, 3, 6, 4, 3) else print("wrong species")
    ...
    for (c in 1:length(Coastal_Countries)) {
        ...
        for (y in 1:nrow(FSR_annual)) {
            Coastal_Catch[which(Coastal_Catch[, 1] == FSR_annual$Year[y]), Area_Flag[c] + 1] <- Coastal_Catch[which(Coastal_Catch[, 1] == FSR_annual$Year[y]), 
                Area_Flag[c] + 1] + FSR_annual$mt[y]/4
        }
    }
    
    if (sum(Coastal_Catch[(nrow(LL_Catch) - 3):nrow(LL_Catch), 2:(n_areas + 1)]) == 0) {
        Coastal_Catch[(nrow(LL_Catch) - 3):nrow(LL_Catch), 2:(n_areas + 1)] <- Coastal_Catch[(nrow(LL_Catch) - 3 - 4):(nrow(LL_Catch) - 4), 2:(n_areas + 
            1)]
    }
