...
ll_catch = function(Grid_Catch, FSR_Catch, Species, last_year, dir) {
    ...
    for (c in 1:length(Countries)) {
        ...
        Grid_weight_quarterly <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% mutate(YQ = Yrr + 
            (Quarter - 1)/4) %>% group_by(YQ, NewAreas) %>% summarise(Tot_Weight_Sum = sum(SumOfWeight, na.rm = TRUE)) %>% spread(key = NewAreas, 
            value = Tot_Weight_Sum, fill = 0)
        
        Grid_number_quarterly <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% mutate(YQ = Yrr + 
            (Quarter - 1)/4) %>% group_by(YQ, NewAreas) %>% summarise(Tot_Number_Sum = sum(SumOfNumber, na.rm = TRUE)) %>% spread(key = NewAreas, 
            value = Tot_Number_Sum, fill = 0)
        
        Grid_number_annual <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% group_by(Yrr, NewAreas) %>% 
            summarise(Tot_Number_Sum = sum(SumOfNumber, na.rm = TRUE)) %>% group_by(Yrr) %>% mutate(EPO = sum(Tot_Number_Sum, 
            na.rm = TRUE)) %>% spread(key = NewAreas, value = Tot_Number_Sum, fill = 0)
        
        Grid_weight_annual <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% group_by(Yrr, NewAreas) %>% 
            summarise(Tot_Weight_Sum = sum(SumOfWeight, na.rm = TRUE)) %>% group_by(Yrr) %>% mutate(EPO = sum(Tot_Weight_Sum, 
            na.rm = TRUE)) %>% spread(key = NewAreas, value = Tot_Weight_Sum, fill = 0)
        
        # calculate the prop (in weight and number) by quarter by area
        Grid_prop_quarterly <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% group_by(Yrr, Quarter, 
            NewAreas) %>% summarise(Tot_Weight_Sum = sum(SumOfWeight, na.rm = TRUE)) %>% group_by(Yrr) %>% mutate(Prop = Tot_Weight_Sum/sum(Tot_Weight_Sum, 
            na.rm = TRUE)) %>% select(Yrr, Quarter, NewAreas, Prop) %>% spread(key = NewAreas, value = Prop, fill = 0) %>% mutate(YQ = Yrr + 
            (Quarter - 1)/4)
        
        Grid_prop_num_quarterly <- Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>% group_by(Yrr, Quarter, 
            NewAreas) %>% summarise(Tot_Number_Sum = sum(SumOfNumber, na.rm = TRUE)) %>% group_by(Yrr) %>% mutate(Prop = Tot_Number_Sum/sum(Tot_Number_Sum, 
            na.rm = TRUE)) %>% select(Yrr, Quarter, NewAreas, Prop) %>% spread(key = NewAreas, value = Prop, fill = 0) %>% mutate(YQ = Yrr + 
            (Quarter - 1)/4)
        
        # if the data do not include some areas for the entire period, add 0 catch to those areas to make the format of the dataset
        ...
        # flag = 0 (have gridded number data); 1 (gridded weight data < FSR, need to do allocation); 2 (gridded weight data > FSR,
        # use gridded weight data); 3 (no gridded number or weight data, need to do allowcation);
        ...
        if (Countries[c] %in% Special_Countires) {
            allocation_flag[flag_id == TRUE] <- ifelse(Grid_number_annual$EPO > 0, ifelse(Grid_weight_annual$EPO > 0 & Grid_weight_annual$EPO < 
                FSR_annual[which(FSR_annual$Year %in% Grid_weight_annual$Yrr), "mt"], 1, 0), ifelse(Grid_weight_annual$EPO > 
                0, ifelse(Grid_weight_annual$EPO < FSR_annual[which(FSR_annual$Year %in% Grid_weight_annual$Yrr), "mt"], 1, 
                2), 3))
        } else {
        ...
        for (i in 1:length(FSR_annual$Year)) {
            ...
            if (allocation_flag[i] == 3) {
                ...
                if (sum(Grid_prop_quarterly[paste0("", seq(1, n_areas))]) > 0) {
                  ...
                  prop_num <- Grid_prop_num_quarterly[which(Grid_prop_num_quarterly$Yrr == year_new), c("Quarter", paste0("", 
                    seq(1, n_areas)))]
                  prop_final <- matrix(0, nrow = 4, ncol = n_areas)
                  for (q in 1:4) {
        ...
        save <- right_join(Grid_number_quarterly[, c("YQ", paste0("", seq(1, n_areas)))], allocation[, c(1, seq(4, n_areas + 
            3))])
        ...
        if (sum(!is.na(save[seq(nrow(save) - 3, nrow(save)), 2:(1 + 2 * n_areas)])) == 0) {
            # print(Countries[c])
            save[seq(nrow(save) - 3, nrow(save)), 2:(1 + 2 * n_areas)] <- save[seq(nrow(save) - 3 - 4, nrow(save) - 4), 2:(1 + 
                2 * n_areas)]
        }
