...
LLcatch = function(Grid_Catch,FSR_Catch,Species,last_year) {
...
for (c in 1:length(Countries)) {# deal with the allocation by country
  ...
  if(Countries[c] %in% Special_Countires) {
    allocation_flag[flag_id==TRUE] <- ifelse(Grid_number_annual$EPO>0,ifelse(Grid_weight_annual$EPO>0&Grid_weight_annual$EPO<FSR_annual[which(FSR_annual$Year %in% Grid_weight_annual$Yrr),"mt"],1,0),
  ...
  for (i in 1:length(FSR_annual$Year)) { # do the allocation for each year
    ...
    if(is.na(as.numeric(FSR_annual[which(FSR_annual$Year==year),"mt"]))==FALSE) FSR <- as.numeric(FSR_annual[which(FSR_annual$Year==year),"mt"])
    ...
    if (allocation_flag[i]==1) {
      ...
      allocation[which(allocation$Year==year),paste0("W",seq(1,n_areas))] <- FSR * Grid_prop_quarterly[which(allocation$Year==year),paste0("",seq(1,n_areas))]
    }
    
    if (allocation_flag[i]==2) {
      # use the gridded weight data directly
      allocation[which(allocation$Year==year),paste0("W",seq(1,n_areas))] <- Grid_weight_quarterly[which(floor(Grid_weight_quarterly$YQ)==year),paste0("",seq(1,n_areas))]
    }
...
if(Species=="BET") Area_Flag <- c(6,3,3,4,3,3,6,4,3)
...
for (c in 1:length(Coastal_Countries)) {
  ...
  for (y in 1:nrow(FSR_annual)) {
    Coastal_Catch[which(Coastal_Catch[,1]==FSR_annual$Year[y]),Area_Flag[c]+1] <- Coastal_Catch[which(Coastal_Catch[,1]==FSR_annual$Year[y]),Area_Flag[c]+1] + FSR_annual$mt[y]/4
  }
}

if(sum(Coastal_Catch[(nrow(LL_Catch)-3):nrow(LL_Catch),2:(n_areas+1)])==0) {
  Coastal_Catch[(nrow(LL_Catch)-3):nrow(LL_Catch),2:(n_areas+1)] <- Coastal_Catch[(nrow(LL_Catch)-3-4):(nrow(LL_Catch)-4),2:(n_areas+1)]
}
