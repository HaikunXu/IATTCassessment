...
LLcatch = function(Grid_Catch,
  ...
  Grid_Catch$NewAreas <-
    AreaCode(Grid_Catch$Lat, Grid_Catch$Lon, Species = Species)
  data <-
    data.frame(
      "Lat" = Grid_Catch$Lat,
      "Lon" = Grid_Catch$Lon,
      "Areas" = as.factor(Grid_Catch$NewAreas)
    )
  
  wmap <- map_data("world")
  ggplot() +
    geom_point(
      aes(x = Lon, y = Lat, color = Areas),
      data = data,
      size = 6,
      shape = 15
    ) +
  ...
  for (c in 1:length(Countries)) {
    ...
    Grid_prop_quarterly <-
      ...
      spread(key = NewAreas,
             value = Prop,
             fill = 0) %>% mutate(YQ = Yrr + (Quarter - 1) / 4)
    
    Grid_prop_num_quarterly <-
      Grid_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>%
    ...
    FSR_annual <-
      FSR_Catch %>% filter(FlagAbv == Countries[c], SpeciesAbv == Species) %>%
    ...
    if (Countries[c] %in% Special_Countires) {
      allocation_flag[flag_id == TRUE] <-
        ifelse(
          Grid_number_annual$EPO > 0,
          ifelse(
            Grid_weight_annual$EPO > 0 &
              Grid_weight_annual$EPO < FSR_annual[which(FSR_annual$Year %in% Grid_weight_annual$Yrr), "mt"],
            1,
            0
          ),
          ifelse(
            Grid_weight_annual$EPO > 0,
            ifelse(Grid_weight_annual$EPO <
                     FSR_annual[which(FSR_annual$Year %in% Grid_weight_annual$Yrr), "mt"], 1, 2),
            3
          )
        )
    }
      ...
      allocation_flag[flag_id == TRUE] <-
        ifelse(Grid_number_annual$EPO > 0,
               0,
               ifelse(
                 Grid_weight_annual$EPO > 0,
                 ifelse(Grid_weight_annual$EPO <
    ...
    allocation <-
      data.frame(
        "YQ" = rep(YQ),
        "Year" = floor(YQ),
        "Quarter" =  rep(1:4, length(YQ) / 4)
      )
    
    allocation_weight <-
      data.frame(matrix(NA, nrow = length(YQ), ncol = n_areas))
    ...
    for (i in 1:length(FSR_annual$Year)) {
      ...
      if (is.na(as.numeric(FSR_annual[which(FSR_annual$Year == year), "mt"])) ==
          FALSE)
        FSR <- as.numeric(FSR_annual[which(FSR_annual$Year == year), "mt"])
      else
        FSR <-
          as.numeric(FSR_annual[which(FSR_annual$Year == (year - 1)), "mt"])
      
      if (allocation_flag[i] == 1) {
        # allocate using FSR and weight prop by quarter by area
        allocation[which(allocation$Year == year), paste0("W", seq(1, n_areas))] <-
          FSR * Grid_prop_quarterly[which(allocation$Year == year), paste0("", seq(1, n_areas))]
      }
      
      if (allocation_flag[i] == 2) {
        # use the gridded weight data directly
        allocation[which(allocation$Year == year), paste0("W", seq(1, n_areas))] <-
          Grid_weight_quarterly[which(floor(Grid_weight_quarterly$YQ) == year), paste0("", seq(1, n_areas))]
      }
        ...
        if (sum(Grid_prop_quarterly[paste0("", seq(1, n_areas))]) > 0) {
          ...
          for (year_diff in 1:40) {
            ...
            if ((year - year_diff) %in% Grid_number_annual$Yrr) {
              if (Grid_weight_annual[which(Grid_weight_annual$Yrr == (year - year_diff)), "EPO"] >
                  0) {
          ...
          prop <-
            Grid_prop_quarterly[which(Grid_prop_quarterly$Yrr == year_new), c("Quarter", paste0("", seq(1, n_areas)))]
          ...
          for (q in 1:4) {
            ...
            if (q %in% prop$Quarter)
              prop_final[q, ] <-
                data.matrix(prop[which(prop$Quarter == q), 2:(n_areas + 1)])
          }
          # print(year_new)
          allocation[which(allocation$Year == year), paste0("W", seq(1, n_areas))] <-
            FSR * prop_final
          ...
          for (year_diff in 1:40) {
            if ((year - year_diff) %in% Grid_number_annual$Yrr) {
              if (Grid_number_annual[which(Grid_number_annual$Yrr == (year - year_diff)), "EPO"] >
                  0) {
                year_new <- year - year_diff
                break
              }
          ...
          prop_num <-
            Grid_prop_num_quarterly[which(Grid_prop_num_quarterly$Yrr == year_new), c("Quarter", paste0("", seq(1, n_areas)))]
          prop_final <- matrix(0, nrow = 4, ncol = n_areas)
          for (q in 1:4) {
            if (q %in% prop_num$Quarter)
              prop_final[q, ] <-
                data.matrix(prop_num[which(prop_num$Quarter == q), 2:(n_areas + 1)])
          }
          # print(year_new)
          allocation[which(allocation$Year == year), paste0("W", seq(1, n_areas))] <-
            FSR * prop_final
    ...
    save <-
      right_join(Grid_number_quarterly[, c("YQ", paste0("", seq(1, n_areas)))], allocation[, c(1, seq(4, n_areas +
                                                                                                        3))])
    
    # if no data is available in the last year, copy the data in the previous year
    if (sum(!is.na(save[seq(nrow(save) - 3, nrow(save)), 2:(1 + 2 * n_areas)])) ==
        0) {
      # print(Countries[c])
      save[seq(nrow(save) - 3, nrow(save)), 2:(1 + 2 * n_areas)] <-
        save[seq(nrow(save) - 3 - 4, nrow(save) - 4), 2:(1 + 2 * n_areas)]
    }
    
    # remove those numbers where weight allocation exist (see rule#2 in the word file)
    save[, paste0("", seq(1, n_areas))] <-
      save[, paste0("", seq(1, n_areas))] * ifelse(is.na(save[, paste0("W", seq(1, n_areas))]), 1, NA)
  ...
  colnames(save_all) <-
    c("YQ", paste0("N", seq(1, n_areas)), paste0("W", seq(1, n_areas)))
  write.csv(save_all, "save_all.csv", row.names = FALSE)
  
  LL_Catch <-
    data.frame(save_all) %>% gather(c(paste0("N", seq(1, n_areas)), paste0("W", seq(1, n_areas))), key = "term", value = "catch") %>%
  ...
  ####################### Coastal countries
  Coastal_Countries <-
    as.character(unique(FSR_Catch$FlagAbv))[!as.character(unique(FSR_Catch$FlagAbv)) %in% Countries]
  ...
  for (c in 1:length(Coastal_Countries)) {
    FSR_annual <-
      FSR_Catch %>% filter(FlagAbv == Coastal_Countries[c], SpeciesAbv == Species) %>%
      group_by(Year) %>% summarise(mt = sum(mt))
    
    for (y in 1:nrow(FSR_annual)) {
      Coastal_Catch[which(Coastal_Catch[, 1] == FSR_annual$Year[y]), Area_Flag[c] +
                      1] <-
        Coastal_Catch[which(Coastal_Catch[, 1] == FSR_annual$Year[y]), Area_Flag[c] +
                        1] + FSR_annual$mt[y] / 4
    }
  ...
  if (sum(Coastal_Catch[(nrow(LL_Catch) - 3):nrow(LL_Catch), 2:(n_areas +
                                                                1)]) == 0) {
    Coastal_Catch[(nrow(LL_Catch) - 3):nrow(LL_Catch), 2:(n_areas + 1)] <-
      Coastal_Catch[(nrow(LL_Catch) - 3 - 4):(nrow(LL_Catch) - 4), 2:(n_areas +
                                                                        1)]
  }
  ...
  # totla catch
  LL_Catch[, paste0("W", seq(1, n_areas))] <-
    LL_Catch[, paste0("W", seq(1, n_areas))] + Coastal_Catch[, 2:(n_areas +
                                                                    1)]
