...
cpue_fit <- function(Path, myreplist, s) {
    cpue <- myreplist$cpue
    ...
    ggsave(file = paste0(Path, "CPUE_Fit1.png"), width = 8, height = 6)
    
    cpue <- cpue %>% mutate(Quarter = (Yr - 1)%%4 + 1)
    
    ggplot(data = cpue) + geom_boxplot(aes(x = factor(Fleet), y = Dev/SE, color = factor(Quarter))) + geom_hline(yintercept = 0) + 
        theme_bw(12) + ylab("CPUE residual / CPUE std error")
    
    ggsave(file = paste0(Path, "CPUE_Fit2.png"), width = 6, height = 6)
